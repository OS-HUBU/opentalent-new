...
const checkRolePermission = (userRoles, path) => {
  if (!userRoles || userRoles.length === 0) {
    return { allowed: false, reason: "您没有权限访问该页面" };
  }
  ...
  if (
    ...
    if (path === "/contribution") {
      return {
        allowed: false,
        reason: "账号未激活，无法查看个人贡献详情，请到个人中心完善信息并进行实名认证！",
      };
    }
  }
  ...
  if (getToken()) {
    ...
    if (to.path === "/login") {
      ...
      if (useUserStore().roles.length === 0) {
        ...
        const { allowed, reason } = checkRolePermission(userRoles, to.path);
        if (!allowed) {
          ElMessage({
            message: reason, // 使用返回的具体原因
            type: "warning",
            duration: 3000,
            showClose: true,
          });
